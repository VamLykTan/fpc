...
begin
  ...
  if not edit[0] and     Edit[1] then begin
    ...
    Plan._File        := Plan.fData.Geraet;
    if plan.fKabel[low(plan.fKabel), 0] = '' then Caption    := Plan.fData.Geraet + '- ' + IntToStr(low(Plan.fKabel)) + '. Datensatz';
...
//    Form2.StatusBar1.Panels.Items[0].Text:='';
    Plan.fKabel[Plan.fData.index, 0] := LabeledEdit0.Caption;
    Plan.fKabel[Plan.fData.index, 1] := ComboBox1.Text;
    {**************************************}
    {**                                  **}
    ...
    for pos := 2 to 5 do begin
      Plan.fKabel[Plan.fData.index, pos] := Info[pos];
    end;
    Plan.fKabel[Plan.fData.index, 6] := ComboBox2.Text;
    Plan.fKabel[Plan.fData.index, 7] := LabeledEdit2.Text;
end;
...
begin
  ...
  if     Edit[0] and not Edit[1] then begin
    ...
    Caption:= IntToStr(Plan.fData.Index) + '|' + IntToStr(length(Plan.fKabel)) + '|' +
              IntToStr(High(Plan.fKabel));
    if Plan.fData.index = (High(Plan.fKabel) + 1) then
      SetLength(Plan.fKabel, High(Plan.fKabel) + 2);
    SetKabel;
    plan.SetKabel;
    inc(Plan.fData.index);
    end;
end;
...
begin
  if OpenDialog1.Execute then begin
    ...
    Plan.GetKabel;
    PlanEdit.StringGrid1.RowCount:= High(Plan.fKabel) + 2;
     for a:= Low(Plan.fKabel) to High(Plan.fKabel) do begin
      for b:= 0 to 7 do begin
        if Plan.fKabel[a, b] <> '' then begin
          PlanEdit.StringGrid1.Cells[b+1, a+1]:= Plan.fKabel[a, b];
          if (Plan.fKabel[a, 2] = 'M') then PlanEdit.StringGrid1.Cells[3, a+1]:= 'Mantelleitung';
          if (Plan.fKabel[a, 2] = 'm') then PlanEdit.StringGrid1.Cells[3, a+1]:= 'Mantelleitung';
          if (Plan.fKabel[a, 2] = 'L') then PlanEdit.StringGrid1.Cells[3, a+1]:= 'Schaltlitze';
          if (Plan.fKabel[a, 2] = 'l') then PlanEdit.StringGrid1.Cells[3, a+1]:= 'Schaltlitze';
        end;
      end;
    end;
